...
class HTMLGenerator:
    ...
    def generate_html_graph(self, data, output_path, current_item_id="home", current_item_path=None, breadcrumb_path=None):
        """Generate an interactive HTML graph."""
        if breadcrumb_path is None:
            breadcrumb_path = []
        
        # If no path provided, default to item_id (for backward compatibility)
        if current_item_path is None:
            current_item_path = current_item_id

        # Create breadcrumb navigation
        breadcrumb_html = self._create_breadcrumb_html(breadcrumb_path)
        
        # Generate the complete HTML content
        html_content = self._build_html_structure(data, breadcrumb_html, current_item_id, current_item_path)
    ...
    def _build_html_structure(self, data, breadcrumb_html, current_item_id, current_item_path):
...
<body>
    ...
    def _build_content_sections(self, data, current_item_id, current_item_path):
        ...
        <div class=\"section\">
            <div class=\"layer1-container\">
                <div class=\"layer1 new-item placeholder\" data-new-item=\"true\" data-parent-path=\"{current_item_path}\" data-parent-name=\"{current_item_id}\">New item</div>
    ...
    def _build_layer1_section(self, item, color_name, is_first_item=False):
        ...
        if is_time_category:
            ...
            # Use the path field from item which preserves underscores in names
            layer1_data_path = item.get("layer1_path", layer1_id)
    ...
    def _build_layer2_section(self, layer2_item, color_name):
        ...
        if is_time_category:
            ...
            # Use the path field from item which preserves underscores in names
            layer2_data_path = layer2_item.get("path", layer2_id)
